...
name-spaces. This version is compiled to support the proxy back-end.
...
%package fsal-proxy
Summary: The NFS-GANESHA's FSAL dedicated to proxy, provided as a shared object
Group: Applications/System
Requires: %{name}-common = %{version}-%{release}

%description fsal-proxy
...
%configure  'CFLAGS=-I/usr/local/include -L/usr/local/lib' 'LDFLAGS=-L/usr/local/lib -lexecinfo' '--enable-debug-symbols' '--disable-nfsidmap' '--enable-handle-mapping' '--with-rpcal=ONCRPC' 'CC=gcc -g' "CFLAGS=$CFLAGS -g" 'CXXFLAGS=-g'
...
install -m 755 nfs-ganesha.proxy.sysconfig                 $RPM_BUILD_ROOT%{_sysconfdir}/sysconfig/ganesha
%endif
...
%if %{?_with_sharedfsalyes:1}%{!?_with_sharedfsalyes:0}
%if %{?_with_fuse:0}%{!?_with_fuse:1}
rm -f $RPM_BUILD_ROOT%{_libdir}/libfsalproxy.la
rm -f $RPM_BUILD_ROOT%{_libdir}/libfsalproxy.a
%endif
%endif
